...
def runNotebooks(self, subDirs = True, template = 'nb-tpl-JR', scp = 'nb-sh-JR'):
    ...
    if self.host == 'localhost':
        ...
        if subDirs:
            ...
            Result = self.c.run(f"ls -d -1 '{self.hostDefn[self.host]['nbProcDir'].as_posix()}/'**/* | grep \.out$", warn = True, hide = True)
        else:
...
def tidyNotebooks(self):
    ...
    for n, item in enumerate(self.jobList):
        Result = job.c.run(f"mv {self.hostDefn[self.host]['nbTemplate'].stem}_{n+1}.ipynb {item.name}.ipynb")
        print(Result.stdout)
